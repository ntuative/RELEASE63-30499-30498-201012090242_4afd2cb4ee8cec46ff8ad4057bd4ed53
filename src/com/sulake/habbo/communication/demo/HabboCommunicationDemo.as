package com.sulake.habbo.communication.demo
{
   ...
   public class HabboCommunicationDemo extends Component
   {
      ...
      private var var_2459:PseudoRandom;
      ...
      private var var_206:HabboHotelView;
      
      private var var_1351:String = "";
      
      private const const_1571:String = "fuselogin";
      ...
      {
         if(this.var_46 && this.var_125 && this.var_18 && this._windowManager)
         {
      ...
      private function componentsAreRunning() : void
      {
         ...
         this.var_206 = new HabboHotelView(this._windowManager,assets,Component(context).events);
         var _loc1_:String = this.var_125.getKey("client.hotel_view.image");
         var _loc2_:String = this.var_125.getKey("image.library.url");
         if(_loc1_ != null && _loc2_ != null)
         {
            this.var_206.loadHotelViewImage(_loc2_ + _loc1_);
         }
         var _loc3_:* = this.var_125.getKey("use.sso.ticket") == "1";
         this.var_586 = this.var_125.getKey("sso.ticket",null);
         this.var_1751 = this.var_125.getKey("flash.client.url","");
         this.var_2460 = this.var_125.getKey("external.variables.txt","");
         if(this.var_206 == null)
         {
            this.var_206 = new HabboHotelView(this._windowManager,assets,Component(context).events);
         }
         ...
         else if(this._windowManager != null)
         {
            ...
            if(_loc4_)
            {
               ...
               this._view.addEventListener(HabboLoginDemoView.const_419,this.onInitConnection);
            }
         }
      }
      ...
      private function onInitConnection(param1:Event = null) : void
      {
         ...
         else
         {
            ...
            {
               this.var_46.mode = HabboConnectionType.const_1082;
            }
         }
         this.var_46.initConnection(HabboConnectionType.const_250);
      }
      ...
      private function onConnectionEstablished(param1:Event = null) : void
      {
         ...
         if(_loc2_ != null)
         {
            this.dispatchLoginStepEvent(HabboCommunicationEvent.const_668);
      ...
      private function onInitCrypto(param1:IMessageEvent) : void
      {
         ...
         this.var_2459 = new PseudoRandom(parseInt(_loc4_.substring(_loc4_.length - 4),16),65536);
         switch(this.var_46.mode)
         {
            case HabboConnectionType.const_114:
               _loc6_ = "";
               _loc6_ = this.var_125.getKey("url.prefix",_loc6_);
               _loc6_ = _loc6_.replace("http://","");
               _loc6_ = _loc6_.replace("https://","");
               _loc7_ = this.var_125.getKey("hotelview.banner.url","http:/\nsitename$/gamedata/banner");
               _loc7_ = _loc7_.replace("$sitename$",_loc6_);
               this.var_1351 = _loc4_;
               this.var_206.loadBannerImage(_loc7_ + "?token=" + this.var_1351,this.onHotelViewBannerLoaded);
               break;
            case HabboConnectionType.const_1082:
               this.sendConnectionParameters(_loc2_);
               break;
            case HabboConnectionType.const_1498:
               this.var_878 = this.generateRandomHexString(30);
               this.var_470 = new DiffieHellman(new BigInteger(this.var_125.getKey("connection.development.prime"),16),new BigInteger(this.var_125.getKey("connection.development.generator"),16));
               this.var_470.init(this.var_878);
               _loc8_ = this.var_470.getPublicKey(10);
      ...
      private function onSecretKeyEvent(param1:IMessageEvent) : void
      {
         ...
         this.var_470.generateSharedKey(_loc4_,10);
         var _loc5_:String = this.var_470.getSharedKey(16).toUpperCase();
         var _loc6_:RC4_R27 = new RC4_R27(null,null);
         var _loc7_:IEncryption = new RC4_R27(_loc6_,this.var_2459);
      ...
      private function sendConnectionParameters(param1:IConnection) : void
      {
         ...
         connection.send(new VersionCheckMessageComposer(401,this.var_1751,this.var_2460));
         var machineId:String = "";
         try
         {
            so = SharedObject.getLocal(this.const_1571,"/");
      ...
      private function onSessionParamsEvent(param1:IMessageEvent) : void
      {
         ...
         if(this.var_586 != null)
         {
            _loc4_ = new SSOTicketMessageComposer(this.var_586);
            _loc2_.send(_loc4_);
      ...
      private function onConnectionDisconnected(param1:Event) : void
      {
         ...
         if(this.var_1350)
         {
            this.dispatchLoginStepEvent(HabboCommunicationEvent.const_163);
         }
         if(param1.type == Event.CLOSE && !this.var_2461)
         {
            _loc2_ = this.var_125.getKey("logout.disconnect.url");
            if(this.var_125.keyExists("facebook.user"))
            {
      ...
      private function onDisconnectReason(param1:DisconnectReasonEvent) : void
      {
         ...
         this.var_2461 = true;
         var _loc2_:String = this.var_125.getKey("logout.url");
         if(_loc2_.length > 0)
         {
      ...
      private function setOriginProperty(param1:String) : String
      {
         ...
         {
            return param1.replace("%origin%",this.var_125.getKey("flash.client.origin","popup"));
         }
         return param1;
      }
      ...
      private function onHotelViewBannerLoaded(param1:BitmapData) : void
      {
         ...
         var _loc4_:String = xor(_loc3_,this.var_1351);
         ...
         this.var_470 = new DiffieHellman(_loc10_,_loc11_);
         var _loc13_:int = 10;
         var _loc14_:* = null;
         while(_loc13_ > 0)
         {
            _loc14_ = this.generateRandomHexString(30);
            this.var_470.init(_loc14_);
            _loc15_ = this.var_470.getPublicKey(10);
            if(_loc15_.length >= 64)
            {
               _loc12_ = _loc15_;
               this.var_878 = _loc14_;
            }
