package com.sulake.habbo.communication.demo
{
   ...
   public class HabboLoginDemoView extends EventDispatcher
   {
      ...
      public static const const_419:String = "InitConnection";
       
      
      private var var_166:HabboCommunicationDemo;
      ...
      {
         this.var_166.windowManager.removeWindow("habbo_login_window");
      }
      ...
      {
         var _loc1_:XmlAsset = (this.var_166 as Component).assets.getAssetByName("login_window") as XmlAsset;
         if(_loc1_ == null)
         {
            Logger.log("Could  not find embedded Window XML for Login Window, login_window");
            return;
         }
         this._window = this.var_166.windowManager.createWindow("habbo_login_window","Login",HabboWindowType.const_116,HabboWindowStyle.const_37,0 | 0,new Rectangle(10,10,320,240),null) as IFrameWindow;
         var _loc2_:XML = _loc1_.content as XML;
         var _loc3_:IWindowContainer = this.var_166.windowManager.buildFromXML(_loc2_) as IWindowContainer;
         ...
         var _loc7_:SharedObject = SharedObject.getLocal(this.const_1571,"/");
         if(_loc5_ != null)
         {
         ...
         if(this.var_166.habboConfiguration)
         {
            if(this.var_166.habboConfiguration.getKey("use.sso","false") == "true")
            {
               _loc8_.select();
            }
         }
      ...
      public function populateUserList(param1:Map) : void
      {
         ...
         var _loc3_:XmlAsset = this.var_166.assets.getAssetByName("login_user_list_item") as XmlAsset;
         var _loc4_:IWindow = this.var_166.windowManager.buildFromXML(_loc3_.content as XML);
      ...
      private function windowEventProcessor(param1:WindowEvent = null, param2:IWindow = null) : void
      {
         ...
         if(_loc3_ != null)
         {
            ...
            _loc6_ = SharedObject.getLocal(this.const_1571,"/");
      ...
      private function listEventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this.var_166.sendTryLogin(this.name,this.password,param2.id);
      }
      ...
      private function initSSOTicket() : void
      {
         ...
         {
            this.var_167.removeEventListener(SSOTicket.const_348,this.onParsedTicketSuccess);
            this.var_167.removeEventListener(SSOTicket.const_293,this.onParsedTicketFailure);
            this.var_167 = null;
         }
         var _loc1_:String = "";
         if(this.var_166.habboConfiguration != null)
         {
            _loc1_ = this.var_166.habboConfiguration.getKey("url.prefix",_loc1_);
            _loc1_ = _loc1_.replace("http://","");
            _loc1_ = _loc1_.replace("https://","");
         }
         var _loc2_:IHabboWebLogin = this.var_166.habboCommunication.habboWebLogin(this.name,this.password);
         this.var_167 = new SSOTicket(this.var_166.assets,_loc2_,_loc1_);
         this.var_167.addEventListener(SSOTicket.const_348,this.onParsedTicketSuccess);
         this.var_167.addEventListener(SSOTicket.const_293,this.onParsedTicketFailure);
      }
      
      private function onParsedTicketSuccess(param1:Event) : void
      {
         Logger.log("Got ticket: " + this.var_167.ticket);
         this.var_166.shockwaveClientUrl = this.var_167.shockwaveClientUrl;
         this.var_166.ssoTicket = this.var_167.ticket;
         this.var_167.removeEventListener(SSOTicket.const_348,this.onParsedTicketSuccess);
         this.var_167.removeEventListener(SSOTicket.const_293,this.onParsedTicketFailure);
         this.var_167 = null;
         dispatchEvent(new Event(const_419));
