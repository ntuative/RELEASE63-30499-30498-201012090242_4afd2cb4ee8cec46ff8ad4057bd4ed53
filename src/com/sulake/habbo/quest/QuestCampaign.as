package com.sulake.habbo.quest
{
   ...
   public class QuestCampaign implements IDisposable
   {
      ...
      private var var_263:IItemListWindow;
      ...
      private function reset() : void
      {
         ...
         if(this.var_302)
         {
            for each(_loc1_ in this.var_302)
            {
               this.var_263.removeListItem(_loc1_.view);
               _loc1_.dispose();
            }
            this.var_302 = null;
            this.var_263.destroyListItems();
            this.var_263.removeListItems();
         }
         this.var_444 = null;
      }
      ...
      private function prepareWindow() : void
      {
         ...
         this._window.center();
         this.var_1002 = this._window.findChildByName("quest.container") as IWindowContainer;
         this.var_1252 = this._window.findChildByName("currency.container") as IWindowContainer;
         this.var_263 = this._window.findChildByName("quests") as IItemListWindow;
         var _loc1_:* = "icon_" + this.id + "_pixeltype";
         var _loc2_:IBitmapWrapperWindow = this.var_1252.findChildByName("currency.icon") as IBitmapWrapperWindow;
         this._controller.questEngine.setImageFromAsset(_loc2_,_loc1_,this.onPreviewImageReady);
         this.var_1252.findChildByName("currency.amount").caption = "${quest_" + this.id + ".currency}";
         ...
         this.var_1003 = this._window.findChildByName("help.button") as IWindowContainer;
         if(this.var_1003)
         {
            this.var_1003.addEventListener(WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK,this.helpButtonEventProc);
            this.var_1003.addEventListener(WindowMouseEvent.WINDOW_EVENT_MOUSE_OVER,this.helpButtonEventProc);
            this.var_1003.addEventListener(WindowMouseEvent.const_25,this.helpButtonEventProc);
         }
      }
      ...
      private function openHelp() : void
      {
         ...
         if(this.var_170)
         {
            this.var_1002.addChild(this.var_170.view);
         }
      }
      
      public function updatePurse() : void
      {
         var _loc1_:int = this._controller.questEngine.getActivityPointsForType(this.var_1797);
      ...
      private function onPreviewImageReady(param1:AssetLoaderEvent) : void
      {
         ...
         if(_loc2_ != null)
         {
            _loc3_ = this.var_1252.findChildByName("currency.icon") as IBitmapWrapperWindow;
      ...
      public function openCampaign(param1:Array) : void
      {
         ...
         {
            this.var_302.push(new Quest(_loc2_,this));
         }
         _loc3_ = null;
         for each(_loc4_ in this.var_302)
         {
            this.var_263.addListItem(_loc4_.view);
            if(!_loc3_ && (_loc4_.status == QuestStatusEnum.const_40 || _loc4_.status == QuestStatusEnum.const_397))
            {
               _loc3_ = _loc4_;
            }
         }
         if(_loc3_)
         {
            ...
            if(this.var_263.maxScrollV > 0)
            {
               this.var_263.scrollV = _loc3_.view.y / this.var_263.maxScrollV;
            }
         }
         else if(this.var_302.length > 0)
         {
            this.activateListItem(this.var_302[0]);
         }
      }
      ...
      private function showQuestData(param1:Quest) : void
      {
         ...
         this.var_1002.addChild(this.var_264.view);
         this.var_264.init();
      }
      ...
      {
         this.modifyQuestData(param1);
         this.var_655.push(new QuestCompleted(this,param1));
      }
      ...
      public function closeQuestCompleted(param1:QuestCompleted) : void
      {
         ...
         var _loc3_:* = null;
         var _loc4_:int = 0;
         for each(_loc3_ in this.var_655)
         {
            if(_loc3_ == param1)
            {
               _loc2_ = _loc3_;
         ...
         {
            _loc2_.dispose();
            _loc4_ = this.var_655.indexOf(_loc2_);
            if(_loc4_ >= 0)
            {
               this.var_655.splice(_loc4_,1);
            }
         }
      }
      ...
      private function modifyQuestData(param1:QuestMessageData) : void
      {
         ...
         if(_loc2_)
         {
            ...
            this.var_302[_loc3_] = _loc5_;
            _loc6_ = this.var_263.getListItemIndex(_loc2_.view);
            this.var_263.removeListItem(_loc2_.view);
            if(_loc6_ > -1)
            {
               this.var_263.addListItemAt(_loc5_.view,_loc6_);
            }
            else
            {
               this.var_263.addListItem(_loc5_.view);
            }
