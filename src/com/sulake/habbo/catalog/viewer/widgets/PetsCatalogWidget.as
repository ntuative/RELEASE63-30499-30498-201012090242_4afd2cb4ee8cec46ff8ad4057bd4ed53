package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      override public function init() : void
      {
         ...
         this.var_900 = this.getPetTypeIndexFromProduct(_loc5_.localizationId);
         this.var_335 = [];
         this.var_596 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc9_ = _loc8_.avatarRenderManager.petDataManager.getPetData(this.var_900,this);
               if(_loc9_)
               {
                  this.var_596 = _loc9_.sellableBreeds;
         ...
         var _loc6_:int = 0;
         while(_loc6_ < this.var_596.length)
         {
            _loc10_ = this.var_596[_loc6_] as IBreed;
            _loc3_.push(_loc4_.getKey(this.getRaceLocalizationKey(this.var_900,_loc10_.id),this.getRaceLocalizationKey(this.var_900,_loc10_.id)));
            _loc6_++;
         }
         this._offers.add(this.var_900,_loc5_);
         ...
         events.addEventListener(WidgetEvent.const_1100,this.onApproveNameResult);
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         if(this.var_479 < 0 || this.var_479 > this.var_335.length)
         {
            this.var_479 = 0;
         }
         this.updateImage();
      ...
      private function updateImage() : void
      {
         ...
         if(this.var_479 >= 0 && this.var_479 < this.var_335.length)
         {
            _loc7_ = this.var_335[this.var_479] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = this.var_596[this.var_1799] as IBreed;
         if(!_loc3_)
         {
            return;
         }
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(this.var_900,_loc3_.id,_loc2_,AvatarScaleType.const_47,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_280,AvatarAction.const_715);
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(this.var_479 >= this.var_335.length)
         {
            return "";
         }
         var petColor:IPetColor = this.var_335[this.var_479] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = this.var_596[this.var_1799] as IBreed;
