package com.sulake.habbo.room.object.visualization.room
{
   import com.sulake.habbo.room.object.visualization.room.mask.PlaneMaskManager;
   import com.sulake.habbo.room.object.visualization.room.rasterizer.IPlaneRasterizer;
   import com.sulake.habbo.room.object.visualization.room.rasterizer.animated.LandscapeRasterizer;
   import com.sulake.habbo.room.object.visualization.room.rasterizer.basic.FloorRasterizer;
   import com.sulake.habbo.room.object.visualization.room.rasterizer.basic.WallAdRasterizer;
   import com.sulake.habbo.room.object.visualization.room.rasterizer.basic.WallRasterizer;
   import com.sulake.room.object.visualization.IRoomObjectVisualizationData;
   import com.sulake.room.object.visualization.utils.IGraphicAssetCollection;
   
   public class RoomVisualizationData implements IRoomObjectVisualizationData
   {
       
      
      private var var_466:WallRasterizer;
      
      private var var_468:FloorRasterizer;
      
      private var var_710:WallAdRasterizer;
      
      private var var_467:LandscapeRasterizer;
      
      private var var_711:PlaneMaskManager;
      
      private var var_734:Boolean = false;
      
      public function RoomVisualizationData()
      {
         super();
         this.var_466 = new WallRasterizer();
         this.var_468 = new FloorRasterizer();
         this.var_710 = new WallAdRasterizer();
         this.var_467 = new LandscapeRasterizer();
         this.var_711 = new PlaneMaskManager();
      }
      
      public function get initialized() : Boolean
      {
         return this.var_734;
      }
      
      public function get floorRasterizer() : IPlaneRasterizer
      {
         return this.var_468;
      }
      
      public function get wallRasterizer() : IPlaneRasterizer
      {
         return this.var_466;
      }
      
      public function get wallAdRasterizr() : WallAdRasterizer
      {
         return this.var_710;
      }
      
      public function get landscapeRasterizer() : IPlaneRasterizer
      {
         return this.var_467;
      }
      
      public function get maskManager() : PlaneMaskManager
      {
         return this.var_711;
      }
      
      public function dispose() : void
      {
         if(this.var_466 != null)
         {
            this.var_466.dispose();
            this.var_466 = null;
         }
         if(this.var_468 != null)
         {
            this.var_468.dispose();
            this.var_468 = null;
         }
         if(this.var_710 != null)
         {
            this.var_710.dispose();
            this.var_710 = null;
         }
         if(this.var_467 != null)
         {
            this.var_467.dispose();
            this.var_467 = null;
         }
         if(this.var_711 != null)
         {
            this.var_711.dispose();
            this.var_711 = null;
         }
      }
      
      public function clearCache() : void
      {
         if(this.var_466 != null)
         {
            this.var_466.clearCache();
         }
         if(this.var_468 != null)
         {
            this.var_468.clearCache();
         }
         if(this.var_467 != null)
         {
            this.var_467.clearCache();
         }
      }
      
      public function initialize(param1:XML) : Boolean
      {
         var _loc7_:* = null;
         var _loc8_:* = null;
         var _loc9_:* = null;
         var _loc10_:* = null;
         var _loc11_:* = null;
         this.reset();
         if(param1 == null)
         {
            return false;
         }
         var _loc2_:XMLList = param1.wallData;
         if(_loc2_.length() > 0)
         {
            _loc7_ = _loc2_[0];
            this.var_466.initialize(_loc7_);
         }
         var _loc3_:XMLList = param1.floorData;
         if(_loc3_.length() > 0)
         {
            _loc8_ = _loc3_[0];
            this.var_468.initialize(_loc8_);
         }
         var _loc4_:XMLList = param1.wallAdData;
         if(_loc4_.length() > 0)
         {
            _loc9_ = _loc4_[0];
            this.var_710.initialize(_loc9_);
         }
         var _loc5_:XMLList = param1.landscapeData;
         if(_loc5_.length() > 0)
         {
            _loc10_ = _loc5_[0];
            this.var_467.initialize(_loc10_);
         }
         var _loc6_:XMLList = param1.maskData;
         if(_loc6_.length() > 0)
         {
            _loc11_ = _loc6_[0];
            this.var_711.initialize(_loc11_);
         }
         return true;
      }
      
      public function initializeAssetCollection(param1:IGraphicAssetCollection) : void
      {
         if(this.var_734)
         {
            return;
         }
         this.var_466.initializeAssetCollection(param1);
         this.var_468.initializeAssetCollection(param1);
         this.var_710.initializeAssetCollection(param1);
         this.var_467.initializeAssetCollection(param1);
         this.var_711.initializeAssetCollection(param1);
         this.var_734 = true;
      }
      
      protected function reset() : void
      {
      }
   }
}
